...
public class BOActivityIndicator: UIView, CAAnimationDelegate {
    ...
    @IBInspectable var strokeColor: UIColor = UIColor(red: 0.0, green: 122.0/255.0, blue: 1.0, alpha: 1.0){
        didSet{
    ...
    @IBInspectable var fontSize: Float = 30 {
        didSet{
    ...
    var hideAfterTime: TimeInterval = dhHidesWhenCompletedDelay
    private(set) var status: ProgressStatus = .Unknown
    ...
    public func animationDidStop(_: CAAnimation, finished flag: Bool) {
        if hidesWhenCompleted {
            Timer.scheduledTimer(timeInterval: dhHidesWhenCompletedDelay, target: self, selector: #selector(BOActivityIndicator.hiddenLoadingView), userInfo: nil, repeats: false)
        } else {
            status = .Completion
    ...
    private func initialize() {
        ...
        NotificationCenter.default.addObserver(self, selector:#selector(BOActivityIndicator.resetAnimations), name: NSNotification.Name.UIApplicationDidBecomeActive, object: nil)
    }
